...
export default function TasksScreen() {
  const { tasks, addTask, updateTask, deleteTask, clearCompletedTasks } = useTasks();
  ...
  const hasCompleted = uiTasks.some(t => t.completed);
    ...
    <FadeScreen>
      <ThemedView style={styles.container}>
        <ThemedView style={styles.header}>
          ...
          {hasCompleted && (
            <Pressable onPress={handleClearDone} style={[styles.clearBtn, { borderColor }]}>
        ...
        <View style={styles.filterRow}>
          {(['all', 'mine'] as const).map(f => (
            <Pressable key={f} onPress={() => setWhoFilter(f)}
              style={[styles.filterChip, { borderColor, backgroundColor: cardBg }, whoFilter === f && { backgroundColor: primaryColor, borderColor: primaryColor }]}>
          ...
          {(['all', 'active', 'completed'] as const).map(f => (
            <Pressable key={f} onPress={() => setStatusFilter(f)}
              style={[styles.filterChip, { borderColor, backgroundColor: cardBg }, statusFilter === f && { backgroundColor: primaryColor, borderColor: primaryColor }]}>
        ...
        {filtered.length === 0 ? (
          <ThemedEmptyState
            ...
            icon={<CheckCircle size={64} color="#106d8f" opacity={0.5} />}
